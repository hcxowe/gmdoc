...
//            '<div class="fixed-table-toolbar"></div>',
    ...
    BootstrapTable.prototype.initToolbar = function () {
        ...
        if (this.options.showColumns) {
            html.push(sprintf('<div class="keep-open btn-group" title="%s">',
                ...
                '<button type="button" style = "border-radius : 0px" class="btn btn-default' + (this.options.iconSize == undefined ? '' : ' btn-' + this.options.iconSize) + ' dropdown-toggle" data-toggle="dropdown">',
    ...
    BootstrapTable.prototype.initPagination = function () {
        ...
        var pageNumber = [
                ...
                '<button type="button" style = "font-size :13px;padding: 4px 10px;border-radius : 0px" class="btn btn-default ' + (this.options.iconSize === undefined ? '' : ' btn-' + this.options.iconSize) + ' dropdown-toggle" data-toggle="dropdown">',
...
//            this.$pagination[this.getData().length ? 'show' : 'hide']();
            //fix ie下.fixed-table-pagination的占据了比较大的空间，导致表格下部会出现大的空白
            if (this.$pagination.find('div.pagination').is(':hidden') && this.$pagination.find('span.page-list').is(':hidden')){
            	this.$pagination.hide();
            }else{
            	this.$pagination.show();
            }
...
//            'margin-right': scrollWidth
//        });
        $fixedHeader.find('table').css('width', that.$el.css('width'))
